...
//			complex_accum_t acmax_o,
    ...
    #pragma HLS ARRAY_PARTITION variable=auto_corr1_reg complete
...
// #pragma HLS ALLOCATION instances=mul limit=4 operation
        ...
        complex_data_t reg0;
        reg0 = (n < N) ? complex_data_t{0, 0} : reg[0]; // Handle initial condition
        complex_data_t reg0_conj = std::conj(reg0);

        // complex_data_t reg0_conj;
        // reg0_conj = (n < N) ? complex_data_t{0, 0} : std::conj(reg[0]); // Handle initial condition
        ...
        // Compute difference
        complex_rs_t auto_corr1_reg0 = (n < N) ? complex_rs_t{0, 0} : auto_corr1_reg[0]; // Handle initial condition
        complex_rs_t rs = auto_corr1 - auto_corr1_reg0;

        // Optimized register shifting with partial unrolling
